// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: msgmove.proto

#define INTERNAL_SUPPRESS_PROTOBUF_FIELD_DEPRECATION
#include "msgmove.pb.h"

#include <algorithm>

#include <google/protobuf/stubs/common.h>
#include <google/protobuf/stubs/once.h>
#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/wire_format_lite_inl.h>
#include <google/protobuf/descriptor.h>
#include <google/protobuf/generated_message_reflection.h>
#include <google/protobuf/reflection_ops.h>
#include <google/protobuf/wire_format.h>
// @@protoc_insertion_point(includes)

namespace proto {

namespace {

const ::google::protobuf::Descriptor* msgmove_descriptor_ = NULL;
const ::google::protobuf::internal::GeneratedMessageReflection*
  msgmove_reflection_ = NULL;

}  // namespace


void protobuf_AssignDesc_msgmove_2eproto() {
  protobuf_AddDesc_msgmove_2eproto();
  const ::google::protobuf::FileDescriptor* file =
    ::google::protobuf::DescriptorPool::generated_pool()->FindFileByName(
      "msgmove.proto");
  GOOGLE_CHECK(file != NULL);
  msgmove_descriptor_ = file->message_type(0);
  static const int msgmove_offsets_[6] = {
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, sx_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, sy_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, speed_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, ex_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, ey_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, delay_),
  };
  msgmove_reflection_ =
    new ::google::protobuf::internal::GeneratedMessageReflection(
      msgmove_descriptor_,
      msgmove::default_instance_,
      msgmove_offsets_,
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, _has_bits_[0]),
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(msgmove, _unknown_fields_),
      -1,
      ::google::protobuf::DescriptorPool::generated_pool(),
      ::google::protobuf::MessageFactory::generated_factory(),
      sizeof(msgmove));
}

namespace {

GOOGLE_PROTOBUF_DECLARE_ONCE(protobuf_AssignDescriptors_once_);
inline void protobuf_AssignDescriptorsOnce() {
  ::google::protobuf::GoogleOnceInit(&protobuf_AssignDescriptors_once_,
                 &protobuf_AssignDesc_msgmove_2eproto);
}

void protobuf_RegisterTypes(const ::std::string&) {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedMessage(
    msgmove_descriptor_, &msgmove::default_instance());
}

}  // namespace

void protobuf_ShutdownFile_msgmove_2eproto() {
  delete msgmove::default_instance_;
  delete msgmove_reflection_;
}

void protobuf_AddDesc_msgmove_2eproto() {
  static bool already_here = false;
  if (already_here) return;
  already_here = true;
  GOOGLE_PROTOBUF_VERIFY_VERSION;

  ::google::protobuf::DescriptorPool::InternalAddGeneratedFile(
    "\n\rmsgmove.proto\022\005proto\"W\n\007msgmove\022\n\n\002sx\030"
    "\001 \002(\002\022\n\n\002sy\030\002 \002(\002\022\r\n\005speed\030\003 \002(\002\022\n\n\002ex\030\004"
    " \002(\002\022\n\n\002ey\030\005 \002(\002\022\r\n\005delay\030\006 \001(\003", 111);
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedFile(
    "msgmove.proto", &protobuf_RegisterTypes);
  msgmove::default_instance_ = new msgmove();
  msgmove::default_instance_->InitAsDefaultInstance();
  ::google::protobuf::internal::OnShutdown(&protobuf_ShutdownFile_msgmove_2eproto);
}

// Force AddDescriptors() to be called at static initialization time.
struct StaticDescriptorInitializer_msgmove_2eproto {
  StaticDescriptorInitializer_msgmove_2eproto() {
    protobuf_AddDesc_msgmove_2eproto();
  }
} static_descriptor_initializer_msgmove_2eproto_;

// ===================================================================

#ifndef _MSC_VER
const int msgmove::kSxFieldNumber;
const int msgmove::kSyFieldNumber;
const int msgmove::kSpeedFieldNumber;
const int msgmove::kExFieldNumber;
const int msgmove::kEyFieldNumber;
const int msgmove::kDelayFieldNumber;
#endif  // !_MSC_VER

msgmove::msgmove()
  : ::google::protobuf::Message() {
  SharedCtor();
  // @@protoc_insertion_point(constructor:proto.msgmove)
}

void msgmove::InitAsDefaultInstance() {
}

msgmove::msgmove(const msgmove& from)
  : ::google::protobuf::Message() {
  SharedCtor();
  MergeFrom(from);
  // @@protoc_insertion_point(copy_constructor:proto.msgmove)
}

void msgmove::SharedCtor() {
  _cached_size_ = 0;
  sx_ = 0;
  sy_ = 0;
  speed_ = 0;
  ex_ = 0;
  ey_ = 0;
  delay_ = GOOGLE_LONGLONG(0);
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
}

msgmove::~msgmove() {
  // @@protoc_insertion_point(destructor:proto.msgmove)
  SharedDtor();
}

void msgmove::SharedDtor() {
  if (this != default_instance_) {
  }
}

void msgmove::SetCachedSize(int size) const {
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
}
const ::google::protobuf::Descriptor* msgmove::descriptor() {
  protobuf_AssignDescriptorsOnce();
  return msgmove_descriptor_;
}

const msgmove& msgmove::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_msgmove_2eproto();
  return *default_instance_;
}

msgmove* msgmove::default_instance_ = NULL;

msgmove* msgmove::New() const {
  return new msgmove;
}

void msgmove::Clear() {
#define OFFSET_OF_FIELD_(f) (reinterpret_cast<char*>(      \
  &reinterpret_cast<msgmove*>(16)->f) - \
   reinterpret_cast<char*>(16))

#define ZR_(first, last) do {                              \
    size_t f = OFFSET_OF_FIELD_(first);                    \
    size_t n = OFFSET_OF_FIELD_(last) - f + sizeof(last);  \
    ::memset(&first, 0, n);                                \
  } while (0)

  if (_has_bits_[0 / 32] & 63) {
    ZR_(sx_, ey_);
  }

#undef OFFSET_OF_FIELD_
#undef ZR_

  ::memset(_has_bits_, 0, sizeof(_has_bits_));
  mutable_unknown_fields()->Clear();
}

bool msgmove::MergePartialFromCodedStream(
    ::google::protobuf::io::CodedInputStream* input) {
#define DO_(EXPRESSION) if (!(EXPRESSION)) goto failure
  ::google::protobuf::uint32 tag;
  // @@protoc_insertion_point(parse_start:proto.msgmove)
  for (;;) {
    ::std::pair< ::google::protobuf::uint32, bool> p = input->ReadTagWithCutoff(127);
    tag = p.first;
    if (!p.second) goto handle_unusual;
    switch (::google::protobuf::internal::WireFormatLite::GetTagFieldNumber(tag)) {
      // required float sx = 1;
      case 1: {
        if (tag == 13) {
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   float, ::google::protobuf::internal::WireFormatLite::TYPE_FLOAT>(
                 input, &sx_)));
          set_has_sx();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(21)) goto parse_sy;
        break;
      }

      // required float sy = 2;
      case 2: {
        if (tag == 21) {
         parse_sy:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   float, ::google::protobuf::internal::WireFormatLite::TYPE_FLOAT>(
                 input, &sy_)));
          set_has_sy();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(29)) goto parse_speed;
        break;
      }

      // required float speed = 3;
      case 3: {
        if (tag == 29) {
         parse_speed:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   float, ::google::protobuf::internal::WireFormatLite::TYPE_FLOAT>(
                 input, &speed_)));
          set_has_speed();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(37)) goto parse_ex;
        break;
      }

      // required float ex = 4;
      case 4: {
        if (tag == 37) {
         parse_ex:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   float, ::google::protobuf::internal::WireFormatLite::TYPE_FLOAT>(
                 input, &ex_)));
          set_has_ex();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(45)) goto parse_ey;
        break;
      }

      // required float ey = 5;
      case 5: {
        if (tag == 45) {
         parse_ey:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   float, ::google::protobuf::internal::WireFormatLite::TYPE_FLOAT>(
                 input, &ey_)));
          set_has_ey();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(48)) goto parse_delay;
        break;
      }

      // optional int64 delay = 6;
      case 6: {
        if (tag == 48) {
         parse_delay:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &delay_)));
          set_has_delay();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectAtEnd()) goto success;
        break;
      }

      default: {
      handle_unusual:
        if (tag == 0 ||
            ::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_END_GROUP) {
          goto success;
        }
        DO_(::google::protobuf::internal::WireFormat::SkipField(
              input, tag, mutable_unknown_fields()));
        break;
      }
    }
  }
success:
  // @@protoc_insertion_point(parse_success:proto.msgmove)
  return true;
failure:
  // @@protoc_insertion_point(parse_failure:proto.msgmove)
  return false;
#undef DO_
}

void msgmove::SerializeWithCachedSizes(
    ::google::protobuf::io::CodedOutputStream* output) const {
  // @@protoc_insertion_point(serialize_start:proto.msgmove)
  // required float sx = 1;
  if (has_sx()) {
    ::google::protobuf::internal::WireFormatLite::WriteFloat(1, this->sx(), output);
  }

  // required float sy = 2;
  if (has_sy()) {
    ::google::protobuf::internal::WireFormatLite::WriteFloat(2, this->sy(), output);
  }

  // required float speed = 3;
  if (has_speed()) {
    ::google::protobuf::internal::WireFormatLite::WriteFloat(3, this->speed(), output);
  }

  // required float ex = 4;
  if (has_ex()) {
    ::google::protobuf::internal::WireFormatLite::WriteFloat(4, this->ex(), output);
  }

  // required float ey = 5;
  if (has_ey()) {
    ::google::protobuf::internal::WireFormatLite::WriteFloat(5, this->ey(), output);
  }

  // optional int64 delay = 6;
  if (has_delay()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(6, this->delay(), output);
  }

  if (!unknown_fields().empty()) {
    ::google::protobuf::internal::WireFormat::SerializeUnknownFields(
        unknown_fields(), output);
  }
  // @@protoc_insertion_point(serialize_end:proto.msgmove)
}

::google::protobuf::uint8* msgmove::SerializeWithCachedSizesToArray(
    ::google::protobuf::uint8* target) const {
  // @@protoc_insertion_point(serialize_to_array_start:proto.msgmove)
  // required float sx = 1;
  if (has_sx()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteFloatToArray(1, this->sx(), target);
  }

  // required float sy = 2;
  if (has_sy()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteFloatToArray(2, this->sy(), target);
  }

  // required float speed = 3;
  if (has_speed()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteFloatToArray(3, this->speed(), target);
  }

  // required float ex = 4;
  if (has_ex()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteFloatToArray(4, this->ex(), target);
  }

  // required float ey = 5;
  if (has_ey()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteFloatToArray(5, this->ey(), target);
  }

  // optional int64 delay = 6;
  if (has_delay()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(6, this->delay(), target);
  }

  if (!unknown_fields().empty()) {
    target = ::google::protobuf::internal::WireFormat::SerializeUnknownFieldsToArray(
        unknown_fields(), target);
  }
  // @@protoc_insertion_point(serialize_to_array_end:proto.msgmove)
  return target;
}

int msgmove::ByteSize() const {
  int total_size = 0;

  if (_has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    // required float sx = 1;
    if (has_sx()) {
      total_size += 1 + 4;
    }

    // required float sy = 2;
    if (has_sy()) {
      total_size += 1 + 4;
    }

    // required float speed = 3;
    if (has_speed()) {
      total_size += 1 + 4;
    }

    // required float ex = 4;
    if (has_ex()) {
      total_size += 1 + 4;
    }

    // required float ey = 5;
    if (has_ey()) {
      total_size += 1 + 4;
    }

    // optional int64 delay = 6;
    if (has_delay()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->delay());
    }

  }
  if (!unknown_fields().empty()) {
    total_size +=
      ::google::protobuf::internal::WireFormat::ComputeUnknownFieldsSize(
        unknown_fields());
  }
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = total_size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
  return total_size;
}

void msgmove::MergeFrom(const ::google::protobuf::Message& from) {
  GOOGLE_CHECK_NE(&from, this);
  const msgmove* source =
    ::google::protobuf::internal::dynamic_cast_if_available<const msgmove*>(
      &from);
  if (source == NULL) {
    ::google::protobuf::internal::ReflectionOps::Merge(from, this);
  } else {
    MergeFrom(*source);
  }
}

void msgmove::MergeFrom(const msgmove& from) {
  GOOGLE_CHECK_NE(&from, this);
  if (from._has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    if (from.has_sx()) {
      set_sx(from.sx());
    }
    if (from.has_sy()) {
      set_sy(from.sy());
    }
    if (from.has_speed()) {
      set_speed(from.speed());
    }
    if (from.has_ex()) {
      set_ex(from.ex());
    }
    if (from.has_ey()) {
      set_ey(from.ey());
    }
    if (from.has_delay()) {
      set_delay(from.delay());
    }
  }
  mutable_unknown_fields()->MergeFrom(from.unknown_fields());
}

void msgmove::CopyFrom(const ::google::protobuf::Message& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

void msgmove::CopyFrom(const msgmove& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

bool msgmove::IsInitialized() const {
  if ((_has_bits_[0] & 0x0000001f) != 0x0000001f) return false;

  return true;
}

void msgmove::Swap(msgmove* other) {
  if (other != this) {
    std::swap(sx_, other->sx_);
    std::swap(sy_, other->sy_);
    std::swap(speed_, other->speed_);
    std::swap(ex_, other->ex_);
    std::swap(ey_, other->ey_);
    std::swap(delay_, other->delay_);
    std::swap(_has_bits_[0], other->_has_bits_[0]);
    _unknown_fields_.Swap(&other->_unknown_fields_);
    std::swap(_cached_size_, other->_cached_size_);
  }
}

::google::protobuf::Metadata msgmove::GetMetadata() const {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::Metadata metadata;
  metadata.descriptor = msgmove_descriptor_;
  metadata.reflection = msgmove_reflection_;
  return metadata;
}


// @@protoc_insertion_point(namespace_scope)

}  // namespace proto

// @@protoc_insertion_point(global_scope)
